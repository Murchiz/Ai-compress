...
class ByteTransformer(nn.Module):
    def __init__(self, vocab_size=256, embed_dim=128, num_layers=2, num_heads=4, context_size=128):
        super().__init__()
...
class Predictor:
    ...
    def predict_next_byte_dist(self, context_bytes):
        ...
        x = torch.tensor(context_bytes, dtype=torch.long, device=self.device).unsqueeze(0)
        logits = self.model(x)
    ...
    def train_on_data(self, data_bytes, epochs=1, lr=1e-3):
        ...
        data_tensor = torch.tensor(list(data_bytes), dtype=torch.long, device=self.device)
